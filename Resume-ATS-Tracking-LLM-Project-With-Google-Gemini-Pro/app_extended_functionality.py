...
def match_resume_keywords(resume_text, job_description):
    model = SentenceTransformer('paraphrase-MiniLM-L6-v2')
    resume_embedding = model.encode(resume_text)
    jd_embedding = model.encode(job_description)
    ...
    jd_tokens = re.findall(r'\b\w+\b', job_description.lower())

    # Find common keywords
    matched_keywords = list(set(resume_tokens) & set(jd_tokens))
...
I want the response in ordered list having the structure
1."JD Match":"%.2f"\n
2."MissingKeywords:[]"\n
3."Profile Summary":"\n
4."Matched keywords:[]"
...
if submit:
    if uploaded_file is not None:
        ...
        for match in matched_companies:
            matched=st.write(f"{match['company']}: {match['similarity_score']:.2f}% match")
        print(matched)
